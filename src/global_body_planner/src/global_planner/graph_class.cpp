...
int GraphClass::getNumVertices() { return vertices.size(); }
...
void GraphClass::addEdge(int idx1, int idx2) {
  ...
  g_values[idx2] =
      g_values[idx1] + poseDistance(vertices[idx1], vertices[idx2]);
  return;
}
...
std::vector<int> GraphClass::getSuccessors(int idx) { return successors[idx]; }

void GraphClass::addAction(int idx, Action a) { actions[idx] = a; }

Action GraphClass::getAction(int idx) { return actions[idx]; }
...
void GraphClass::updateGValue(int idx, double val) {
  ...
  for (int successor : getSuccessors(idx)) {
    updateGValue(successor, g_values[idx] + poseDistance(getVertex(idx),
                                                         getVertex(successor)));
  }
}
