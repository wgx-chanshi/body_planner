...
int RRTConnectClass::attemptConnect(State s_existing, State s, double t_s,
                                    State &s_new, Action &a_new,
  ...
  a_new[0] =
      -(2.0 * (3.0 * x_td - 3.0 * x_to + 2.0 * dx_td * t_s + dx_to * t_s)) /
      (t_s * t_s);
  a_new[1] =
      -(2.0 * (3.0 * y_td - 3.0 * y_to + 2.0 * dy_td * t_s + dy_to * t_s)) /
      (t_s * t_s);
  a_new[2] =
      -(2.0 * (3.0 * z_td - 3.0 * z_to + 2.0 * dz_td * t_s + dz_to * t_s)) /
      (t_s * t_s);
  a_new[3] =
      (2.0 * (3.0 * x_td - 3.0 * x_to + dx_td * t_s + 2.0 * dx_to * t_s)) /
      (t_s * t_s);
  a_new[4] =
      (2.0 * (3.0 * y_td - 3.0 * y_to + dy_td * t_s + 2.0 * dy_to * t_s)) /
      (t_s * t_s);
  a_new[5] =
      (2.0 * (3.0 * z_td - 3.0 * z_to + dz_td * t_s + 2.0 * dz_to * t_s)) /
      (t_s * t_s);
  a_new[6] = t_s;
  a_new[7] = t_f;

  a_new[8] =
      -(2.0 * (3.0 * p_td - 3.0 * p_to + 2.0 * dp_td * t_s + dp_to * t_s)) /
      (t_s * t_s);
  a_new[9] =
      (2.0 * (3.0 * p_td - 3.0 * p_to + dp_td * t_s + 2.0 * dp_to * t_s)) /
      (t_s * t_s);
  ...
  if (isValidAction(a_new) == true) {
    ...
    bool isValid =
        (direction == FORWARD)
            ? (isValidStateActionPair(s_start, a_new, terrain, s_new, t_new))
            : (isValidStateActionPairReverse(s_goal, a_new, terrain, s_new,
                                             t_new));
    ...
    if (isValid == true)
      ...
      if (attemptConnect(s_existing, s_new, t_new, s_new, a_new, terrain,
                         direction) == TRAPPED)
        return TRAPPED;
      else
        return ADVANCED;
...
int RRTConnectClass::attemptConnect(State s_existing, State s, State &s_new,
                                    Action &a_new, FastTerrainMap &terrain,
                                    int direction) {
...
int RRTConnectClass::connect(PlannerClass &T, State s, FastTerrainMap &terrain,
                             int direction) {
...
std::vector<Action>
RRTConnectClass::getActionSequenceReverse(PlannerClass &T,
                                          std::vector<int> path) {
...
void RRTConnectClass::postProcessPath(std::vector<State> &state_sequence,
  ...
  while (s != s_goal) {
    ...
    // states left
    while ((attemptConnect(s, s_next, dummy, a_new, terrain, FORWARD) !=
            REACHED) &&
           (s != s_next)) {
      old_state = s_next;
      old_action = a_next;
...
void RRTConnectClass::runRRTConnect(PlannerClass &Ta, PlannerClass &Tb,
                                    FastTerrainMap &terrain) {
  auto t_start = std::chrono::high_resolution_clock::now();
  ...
  while (true) {
    ...
    if (current_elapsed.count() >= anytime_horizon) // 0.3 for plinth
    {
      ...
      // std::cout << "Took too long, retrying with horizon of " <<
      // anytime_horizon << "s" << std::endl;
      return;
    }
    // Generate random s
    ...
    if (isValidState(s_rand, terrain, STANCE)) {
      if (extend(Ta, s_rand, terrain, FORWARD) != TRAPPED) {
        ...
        if (connect(Tb, s_new, terrain, REVERSE) == REACHED) {
          ...
          path_quality_ = Ta.getGValue(Ta.getNumVertices() - 1) +
...
void RRTConnectClass::buildRRTConnect(FastTerrainMap &terrain, State s_start,
                                      State s_goal,
  ...
  while (true) {
    ...
    if (goal_found == true) {
      // Get both paths, remove the back of path_b and reverse it to align with
      // path a
      ...
      std::vector<Action> action_sequence_b =
          getActionSequenceReverse(Tb, path_b);
