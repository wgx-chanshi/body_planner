...
State PlannerClass::randomState(FastTerrainMap &terrain) {
  ...
  std::normal_distribution<double> height_distribution(
      ...
      (z_max_rel - z_min_rel) *
          (1.0 /
           (2 *
            3.0))); // std is such that the max and min are 3 std away from mean
  std::normal_distribution<double> ang_vel_distribution(
      0.0, (P_MAX /
            3.0)); // std is such that the max and min are 3 std away from mean
  ...
  q[2] =
      std::max(std::min(height_distribution(generator), z_max_rel), z_min_rel) +
