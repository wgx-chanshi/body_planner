...
const double V_MAX =
    4.0; // Maximum robot velocity, m/s (4.0 for cheetah, 2.5 for anymal)
const double V_NOM =
    1.5; // Nominal velocity, m/s (used during connect function)
const double P_MAX = 1.0;  // Maximum pitch, rad
const double DP_MAX = 5.0; // Maximum angular velocity in pitch, rad/s
const double ANG_ACC_MAX =
    10.0; // Maximum angular acceleration in pitch, rad/s^2
const double ROBOT_L =
    0.6; // Length of robot body, m (0.6 cheetah, 0.554 ANYmal)
const double ROBOT_W =
    0.256; // Width of robot body, m (0.256 cheetah, 0.232 ANYmal)
const double ROBOT_H = 0.1; // Vertical distance between leg base and bottom of
...
const double KINEMATICS_RES =
    0.05; // Resolution of kinematic feasibility checks, s
const double BACKUP_TIME =
    0.2; // Duration of backup after finding an invalid state, s
const double BACKUP_RATIO =
    0.5; // Ratio of trajectory to back up after finding an invalid state, s
const int NUM_GEN_STATES =
    6; // Number of actions computed for each extend function
const double GOAL_BOUNDS = 0.5; // Distance threshold on reaching the goal (only
