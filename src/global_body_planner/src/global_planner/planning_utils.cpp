...
void getInterpPath(std::vector<State> state_sequence,
  ...
  for (int i = 0; i < action_sequence.size(); i++) {
    interpStateActionPair(state_sequence[i], action_sequence[i], t0, dt,
                          interp_path, interp_t, interp_phase);
...
std::array<double, 3> rotate_grf(std::array<double, 3> surface_norm,
...
State applyStance(State s, Action a, double t) {
  ...
  s_new[0] = x_td + dx_td * t + 0.5 * a_x_td * t * t +
             (a_x_to - a_x_td) * (t * t * t) / (6.0 * t_s);
  s_new[1] = y_td + dy_td * t + 0.5 * a_y_td * t * t +
             (a_y_to - a_y_td) * (t * t * t) / (6.0 * t_s);
  s_new[2] = z_td + dz_td * t + 0.5 * a_z_td * t * t +
             (a_z_to - a_z_td) * (t * t * t) / (6.0 * t_s);
  ...
  s_new[6] = p_td + dp_td * t + 0.5 * a_p_td * t * t +
             (a_p_to - a_p_td) * (t * t * t) / (6.0 * t_s);
  s_new[7] = dp_td + a_p_td * t + (a_p_to - a_p_td) * t * t / (2.0 * t_s);

  return s_new;
}

State applyStance(State s, Action a) { return applyStance(s, a, a[6]); }
...
State applyStanceReverse(State s, Action a, double t) {
  ...
  s_new[3] = dx_to - a_x_td * (t_s - t) -
             (a_x_to - a_x_td) * (t_s * t_s - t * t) / (2.0 * t_s);
  s_new[4] = dy_to - a_y_td * (t_s - t) -
             (a_y_to - a_y_td) * (t_s * t_s - t * t) / (2.0 * t_s);
  s_new[5] = dz_to - a_z_td * (t_s - t) -
             (a_z_to - a_z_td) * (t_s * t_s - t * t) / (2.0 * t_s);
  s_new[0] = x_to - cx * (t_s - t) - 0.5 * a_x_td * (t_s * t_s - t * t) -
             (a_x_to - a_x_td) * (t_s * t_s * t_s - t * t * t) / (6.0 * t_s);
  s_new[1] = y_to - cy * (t_s - t) - 0.5 * a_y_td * (t_s * t_s - t * t) -
             (a_y_to - a_y_td) * (t_s * t_s * t_s - t * t * t) / (6.0 * t_s);
  s_new[2] = z_to - cz * (t_s - t) - 0.5 * a_z_td * (t_s * t_s - t * t) -
             (a_z_to - a_z_td) * (t_s * t_s * t_s - t * t * t) / (6.0 * t_s);

  s_new[7] = dp_to - a_p_td * (t_s - t) -
             (a_p_to - a_p_td) * (t_s * t_s - t * t) / (2.0 * t_s);
  s_new[6] = p_to - cp * (t_s - t) - 0.5 * a_p_td * (t_s * t_s - t * t) -
             (a_p_to - a_p_td) * (t_s * t_s * t_s - t * t * t) / (6.0 * t_s);
...
Action getRandomAction(std::array<double, 3> surf_norm) {
  ...
  // Randomly sample angular acceleration from a normal distribution with low
  // std
  unsigned seed = std::chrono::system_clock::now().time_since_epoch().count();
  std::default_random_engine generator(seed);
  std::normal_distribution<double> ang_acc_distribution(
      0.0, (ANG_ACC_MAX /
            4.0)); // std is such that the max and min are 4 std away from mean
  a[8] = std::max(std::min(ang_acc_distribution(generator), ANG_ACC_MAX),
                  -ANG_ACC_MAX); // 0.0; // for now
  a[9] = std::max(std::min(ang_acc_distribution(generator), ANG_ACC_MAX),
                  -ANG_ACC_MAX); // 0.0;
...
bool isValidState(State s, FastTerrainMap &terrain, int phase) {
  if ((s[0] < terrain.getXData().front()) ||
      (s[0] > terrain.getXData().back()) ||
      (s[1] < terrain.getYData().front()) ||
      (s[1] > terrain.getYData().back()) || (abs(s[6]) >= P_MAX))
    return false;

  if (sqrt(s[3] * s[3] + s[4] * s[4]) >
      V_MAX) // Ignore limit on vertical velocity since this is accurately
             // bounded by gravity
    return false;
  ...
  for (double x_body : test_x) {
    for (double y_body : test_y) {
      ...
      double corner_height =
          z_corner - terrain.getGroundHeight(x_corner, y_corner);
      // std::cout << "x,y,z = (" << x_spatial << ", " << y_spatial << ", " <<
      // z_spatial << "), height = " <<
      //     z_spatial << " - " << terrain.getGroundHeight(x_spatial,y_spatial)
      //     << " = " << height << std::endl;

      // Always check for collision, only check reachability in stance
      if ((corner_height < H_MIN) ||
          ((phase == STANCE) && (leg_height > H_MAX)))
        return false;
    }
  }
  ...
  double height =
      (s[2] + R_33 * z_body) -
      terrain.getGroundHeight(s[0] + R_13 * z_body, s[1] + R_23 * z_body);
  if (height < H_MIN)
    return false;
