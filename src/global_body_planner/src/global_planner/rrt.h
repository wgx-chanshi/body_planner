...
public:
   ...
   */
  virtual int extend(PlannerClass &T, State s, FastTerrainMap &terrain,
                     int direction);
   ...
   */
  void buildRRT(FastTerrainMap &terrain, State s_start, State s_goal,
   ...
   * @param[out] time_to_first_solve The time elapsed until the first valid path
   * was found
   * @param[out] cost_vector Vector of costs each time a better one is found
   * @param[out] cost_vector_times Vector of times at which each better cost is
   * found
   * @param[out] path_duration The duration of the path in seconds
   */
  void getStatistics(double &plan_time, int &success_var,
                     int &vertices_generated, double &time_to_first_solve,
   ...
   * other in the tree
   */
  bool newConfig(State s, State s_near, State &s_new, Action &a_new,
