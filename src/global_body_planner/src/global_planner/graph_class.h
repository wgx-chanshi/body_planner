...
/*!
   This class implements a directed graph data structure with methods for adding
   and deleting vertices and edges, as well storing information at each vertex
   and providing print statements for debugging. Vertices are indexed with ints,
   and edges as unordered maps that map a vertex's index to the indices of its
   parents. If the graph is a tree there should only be one parent per vertex.
   Other unordered maps store information about each vertex, such as the
...
public:
  ...
  /**
   * @brief Retrieve the total number of vertices in the graph, computed as the
  ...
  /**
   * @brief Add an action to a particular vertex. This is the action that lead
   * to this particular vertex from its parent.
   * @param[in] idx Index of the desired vertex
  ...
  /**
   * @brief Initialize the graph by adding the root vertex (idx = 0) and setting
   * g(idx) = 0
