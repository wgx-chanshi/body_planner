...
bool RRTClass::newConfig(State s, State s_near, State &s_new, Action &a_new,
  ...
  for (int i = 0; i < NUM_GEN_STATES; ++i) {
    ...
    for (int j = 0; j < NUM_GEN_STATES; ++j) {
      ...
      if (direction == FORWARD) {
        is_valid =
            isValidStateActionPair(s_near, a_test, terrain, s_test, t_new);
      } else if (direction == REVERSE) {
        is_valid = isValidStateActionPairReverse(s_near, a_test, terrain,
                                                 s_test, t_new);
      }
...
int RRTClass::extend(PlannerClass &T, State s, FastTerrainMap &terrain,
                     int direction) {
...
void RRTClass::getStatistics(double &plan_time, int &success,
...
void RRTClass::buildRRT(FastTerrainMap &terrain, State s_start, State s_goal,
                        std::vector<State> &state_sequence,
                        std::vector<Action> &action_sequence) {
