...
int RRTStarConnectClass::extend(PlannerClass &T, State s,
  ...
  if (newConfig(s, s_nearest, s_new, a_new, terrain, direction) == true) {
    ...
    // Initialize g_s_new
    double g_s_new =
        T.getGValue(s_nearest_index) + poseDistance(s_new, s_nearest);
    for (int i = 0; i < neighbors.size(); i++) {
      int s_near_idx = neighbors[i];
      State s_near = T.getVertex(s_near_idx);

      if (attemptConnect(s_near, s_new, dummy, a_connect, terrain, direction) ==
          REACHED) {
    ...
    for (int s_near_idx : neighbors) {
      if (s_near_idx != s_min_idx) {
        State s_near = T.getVertex(s_near_idx);
        if ((attemptConnect(s_new, s_near, dummy, a_connect, terrain,
                            direction) == REACHED) &&
            (T.getGValue(s_near_idx) >
             (T.getGValue(s_new_idx) + poseDistance(s_near, s_new)))) {
          int s_parent = T.getPredecessor(s_near_idx);
          T.removeEdge(s_parent, s_near_idx);
          T.addEdge(s_new_idx, s_near_idx);
          T.updateGValue(s_near_idx, (T.getGValue(s_new_idx) +
                                      poseDistance(s_near, s_new)));
...
void RRTStarConnectClass::getStateAndActionSequences(
    PlannerClass &Ta, PlannerClass &Tb, int shared_a_idx, int shared_b_idx,
...
void RRTStarConnectClass::buildRRTStarConnect(
    FastTerrainMap &terrain, State s_start, State s_goal,
    std::vector<State> &state_sequence, std::vector<Action> &action_sequence,
    double max_time) {
  ...
  if (goal_found == true) {
    getStateAndActionSequences(Ta, Tb, shared_a_idx, shared_b_idx,
