...
public:
   ...
   * REACHED)
   */
  int attemptConnect(State s_existing, State s, double t_s, State &s_new,
                     Action &a_new, FastTerrainMap &terrain, int direction);
   ...
   * REACHED)
   */
  int attemptConnect(State s_existing, State s, State &s_new, Action &a_new,
   ...
   * @param[in] terrain Height map of the terrain
   */
  void runRRTConnect(PlannerClass &Ta, PlannerClass &Tb,
                     FastTerrainMap &terrain);

  /**
   * @brief Run the full RRT-Connect planner until the goal is found and time
   * has expired, then post process and update statistics
   * @param[in] terrain Height map of the terrain
   ...
   */
  void buildRRTConnect(FastTerrainMap &terrain, State s_start, State s_goal,
...
protected:
  ...
  /// Hard maximum time allowed for the planner, returns unsuccessfully if
  /// reached
